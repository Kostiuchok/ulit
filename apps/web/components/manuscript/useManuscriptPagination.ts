...
export function useManuscriptPagination(content: any, contentWidth: number, contentHeight: number): Result {
  const [pages, setPages] = useState<PageLeaf[] | null>(null);

  useEffect(() => {
    let cancelled = false;
    setPages(null);
    paginateManuscript(content, contentWidth, contentHeight).then((result) => {
