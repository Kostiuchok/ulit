...
export async function uploadManuscriptImageRoute(app: FastifyInstance) {
  app.post(
    "/api/books/:id/manuscript/images",
    ...
    async (request, reply) => {
      ...
      for await (const chunk of data.file) {
        ...
        if (totalSize > MAX_SIZE) throw new AppError("File exceeds 10 MB", 400, "FILE_TOO_LARGE");
        chunks.push(chunk);
      }

      const buffer = Buffer.concat(chunks);
      const ext = data.mimetype === "image/png" ? "png" : data.mimetype === "image/webp" ? "webp" : "jpg";
      const objectName = `public/manuscripts/${id}/${randomUUID()}.${ext}`;
