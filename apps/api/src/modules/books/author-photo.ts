...
// "Автори книги" (T-2060 п.4) is a JSON array on Book (bookAuthors), each
// entry an optional photoUrl -- unlike coverUrl/backCoverUrl/spineUrl this
// has no dedicated DB column to write to (there can be several authors,
// added/removed freely before the author hits "Зберегти зміни"), so this
// route only uploads the file and hands back a URL; the frontend slots it
// into the in-progress author entry the same way pasting a URL already did.
export async function uploadAuthorPhotoRoute(app: FastifyInstance) {
  app.post(
    "/api/books/:id/author-photo",
    ...
    async (request, reply) => {
      ...
      const objectName = `public/author-photos/${id}/${randomUUID()}.${ext}`;
