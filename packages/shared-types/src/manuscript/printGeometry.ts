...
// Title -- top edge -> title's own VERTICAL CENTER (not baseline/top like
// the others): a long title wrapping onto 2-3 lines grows symmetrically
// around this mark instead of just hanging further down from a fixed top,
...
export const TITLE_PAGE_TITLE_FONT_PT = 18; // kept in sync with proseStyles.ts's titlepage-title, though currently unused here -- printHtml.ts's title-centering (transform:translateY) doesn't need to know the font size, unlike every other line
export const TITLE_PAGE_SUBTITLE_FONT_PT = 16;
