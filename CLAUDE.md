...
**Рішення**: `apiFetch` тепер ставить `Content-Type: application/json` для БУДЬ-ЯКОГО методу окрім `GET`/`HEAD`, незалежно від того, чи передано тіло. Кожен уражений роут вже й так очікував порожнє тіло (`const body = (request.body ?? {})` — Fastify-ів дефолтний JSON-парсер сам перетворює порожній рядок на `undefined`) — просто через відсутній Content-Type запит ніколи не долітав до цієї стадії.
